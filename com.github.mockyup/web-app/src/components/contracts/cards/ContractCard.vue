<template>
  <card-container class="project-card" @click="popUpCard">
    <card-body class="d-flex align-items-center">
      <div class="avatar avatar-sm flex-shrink-0">
        <span>{{$filters.subString(contractCard.info.title,0,1)}}</span>
      </div>
      <div class="flex-row ms-2 project-card-description">
        <div>
          <router-link :to="{
            name:'ContractDetail',
            params:{id:contractCard.projectId,contractId:contractCard.id}}">{{contractCard.info.contact?contractCard.info.contact.name+"/":""}}{{contractCard.info.title}}</router-link>
...
export default defineComponent({
  ...
  props: {
    contractCard: { type: Object as PropType<ContractCardInterface> }
  },
  methods: {
    popUpCard () {
      alert('berak')
