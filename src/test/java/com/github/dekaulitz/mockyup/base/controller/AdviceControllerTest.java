...
class AdviceControllerTest extends BaseTest {
    ...
    void handleMethodArgumentNotValid() throws IOException {
        ...
        isTrue(response1.getStatusCode().value() == ResponseCode.VALIDATION_FAIL.getHttpCode().value(), HTTPCODE_NOT_EXPECTED);
        isTrue(response1.getBody().getResponseCode().equals(ResponseCode.VALIDATION_FAIL.getErrorCode()), RESPONSE_BODY_NOT_EXPECTED);
    }
}
