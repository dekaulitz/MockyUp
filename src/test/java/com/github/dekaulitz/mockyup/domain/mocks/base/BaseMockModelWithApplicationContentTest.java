...
class BaseMockModelWithApplicationContentTest {
    ...
    @Test
    void getMockResponseWhenPathIsExactlySameWithOpenApiPath() throws IOException, InvalidMockException, NotFoundException {
        String path = "/books";
        String body = "{}";
        ...
        Mockito.when(this.httpServletRequest.getHeader(APPLICATION_TYPE_ACCEPT)).thenReturn(MediaType.APPLICATION_XML_VALUE);
        MockHelper mockHelper = this.baseMockModel.renderingMockResponse(item, this.httpServletRequest, body, openAPIPaths, paths, openAPI.getComponents());
        Assert.isTrue(mockHelper.getResponseProperty().getHttpCode() == 422, "mockhelper was not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getResponse() != null, "mockhelper was not expected");
        //its static from mocking_test.json
        String result = Helper.parseXmlComponentTest((String) mockHelper.getResponseProperty().getResponse());
        Assert.isTrue(result.equals(Helper.DEFAULT_COMPONENT_EXAMPLE), "mockhelper was not expected");

        Mockito.when(this.httpServletRequest.getHeader(APPLICATION_TYPE_ACCEPT)).thenReturn(MediaType.APPLICATION_JSON_VALUE);
        mockHelper = this.baseMockModel.renderingMockResponse(item, this.httpServletRequest, body, openAPIPaths, paths, openAPI.getComponents());
        Assert.isTrue(mockHelper.getResponseProperty().getResponse().equals(Helper.DEFAULT_COMPONENT_EXAMPLE), "mockhelper was not expected");
    }
    ...
    void getQueryMockResponse() throws IOException, InvalidMockException, NotFoundException {
        ...
        Mockito.when(this.httpServletRequest.getMethod()).thenReturn("get");
        Mockito.when(this.httpServletRequest.getHeader("client-id")).thenReturn("any");
        Mockito.when(this.httpServletRequest.getQueryString()).thenReturn("path=/books?query=title:empty&sort=published:desc");
        //do request with application/xml
        Mockito.when(this.httpServletRequest.getHeader(APPLICATION_TYPE_ACCEPT)).thenReturn(MediaType.APPLICATION_XML_VALUE);
        //test when using example query=title:empty
        MockHelper mockHelper = this.baseMockModel.renderingMockResponse(pathItem, this.httpServletRequest, body, openAPIPaths, paths, openAPI.getComponents());
        ...
        mockHelper = this.baseMockModel.renderingMockResponse(pathItem, this.httpServletRequest, body, openAPIPaths, paths, openAPI.getComponents());
        Assert.notNull(mockHelper, "mockhelper is not expected");
    ...
    void getHeaderMockResponse() throws IOException, InvalidMockException, NotFoundException {
        ...
        MockHelper mockHelper = this.baseMockModel.renderingMockResponse(pathItem, this.httpServletRequest, body, openAPIPaths, paths, openAPI.getComponents());
        Assert.notNull(mockHelper, "mockhelper is not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getResponse().equals(Helper.DEFAULT_COMPONENT_EXAMPLE), "mockhelper is not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getHeaders().get(Helper.DEFAULT_HEADER_NAME).equals(Helper.DEFAULT_HEADER_VALUE), "mock helper is not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getHttpCode() == 400, "mock helper is not expected");
        ...
        Assert.isTrue(mockHelper.getResponseProperty().getResponse().equals(Helper.DEFAULT_COMPONENT_EXAMPLE), "mockhelper is not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getHeaders().get(Helper.DEFAULT_HEADER_NAME).equals(Helper.DEFAULT_HEADER_VALUE), "mock helper is not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getHttpCode() == 200, "mock helper is not expected");
    ...
    void getPathMockResponse() throws Exception {
        ...
        Mockito.when(this.httpServletRequest.getQueryString()).thenReturn("");
        Mockito.when(this.httpServletRequest.getHeader(APPLICATION_TYPE_ACCEPT)).thenReturn(MediaType.APPLICATION_JSON_VALUE);
        MockHelper mockHelper = this.baseMockModel.renderingMockResponse(dtoPathItemOpenApiVmodel.getPathItem(),
    ...
    void getAnotherPathMockResponse() throws Exception {
        ...
        Assert.isTrue(mockHelper.getResponseProperty().getResponse().equals(Helper.DEFAULT_COMPONENT_EXAMPLE), "mockhelper is not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getHeaders().get(Helper.DEFAULT_HEADER_NAME).equals(Helper.DEFAULT_HEADER_VALUE), "mock helper is not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getHttpCode() == 500, "mock helper is not expected");
    ...
    void getAnotherPathMockResponseWithDefault() throws Exception {
        ...
        Assert.isTrue(mockHelper.getResponseProperty().getResponse().equals(Helper.DEFAULT_COMPONENT_EXAMPLE), "mockhelper is not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getHeaders().get(Helper.DEFAULT_HEADER_NAME).equals(Helper.DEFAULT_HEADER_VALUE), "mock helper is not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getHttpCode() == 401, "mock helper is not expected");
    ...
    void getAnotherRequestBoydMockResponse() throws Exception {
        ...
        Assert.isTrue(mockHelper.getResponseProperty().getResponse().equals(Helper.DEFAULT_COMPONENT_EXAMPLE), "mockhelper is not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getHeaders().get(Helper.DEFAULT_HEADER_NAME).equals(Helper.DEFAULT_HEADER_VALUE), "mock helper is not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getHttpCode() == 201, "mock helper is not expected");
    ...
    void getAnotherRequestBoydMockResponseWhenNoDefaultResponseWithQueryPriority() throws Exception {
        ...
        generatePathMockPath(dtoPathItemOpenApiVmodel, this.openAPI);
        Mockito.when(this.httpServletRequest.getMethod()).thenReturn("PUT");
        Mockito.when(this.httpServletRequest.getQueryString()).thenReturn("path=/books/12/update?query=title:empty");
        //do request application/json
      Mockito.when(this.httpServletRequest.getHeader(APPLICATION_TYPE_ACCEPT)).thenReturn(MediaType.APPLICATION_JSON_VALUE);
        MockHelper mockHelper = this.baseMockModel.renderingMockResponse(dtoPathItemOpenApiVmodel.getPathItem(),
                this.httpServletRequest, body, dtoPathItemOpenApiVmodel.getOpenAPIPaths(),
        ...
        Assert.isTrue(mockHelper.getResponseProperty().getResponse().equals(Helper.DEFAULT_COMPONENT_EXAMPLE), "mockhelper is not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getHeaders().get(Helper.DEFAULT_HEADER_NAME).equals(Helper.DEFAULT_HEADER_VALUE), "mock helper is not expected");
        Assert.isTrue(mockHelper.getResponseProperty().getHttpCode() == 422, "mock helper is not expected");
    ...
    private void generatePathMockPath(DtoPathItemOpenApiVmodel dtoPathItemOpenApiVmodel, OpenAPI openApi) throws Exception {
        ...
        for (Map.Entry<String, PathItem> entry : openApi.getPaths().entrySet()) {
            ...
            // every targeting mocks uri should has same length
            if (dtoPathItemOpenApiVmodel.getOpenAPIPaths().length == dtoPathItemOpenApiVmodel.getPaths().length) {
                // converting request path to original for searching data
                for (int i = 0; i < dtoPathItemOpenApiVmodel.getOpenAPIPaths().length; i++) {
                    //checking path parameter
                    if (!dtoPathItemOpenApiVmodel.getOpenAPIPaths()[i].equals(dtoPathItemOpenApiVmodel.getPaths()[i])) {
                        if (dtoPathItemOpenApiVmodel.getOpenAPIPaths()[i].contains("*")) {
                            regexPath[i] = dtoPathItemOpenApiVmodel.getOpenAPIPaths()[i];
                        }
                    } else {
                        //this is for checking wildcard with path parameter
                        if (!dtoPathItemOpenApiVmodel.getPaths()[i].contains("*") && dtoPathItemOpenApiVmodel.getOpenAPIPaths()[i].equals(dtoPathItemOpenApiVmodel.getPaths()[i])) {
                            regexPath[i] = dtoPathItemOpenApiVmodel.getOpenAPIPaths()[i];
                        }
                    }
                }
        ...
        if (!mapped) throw new Exception("invalid type");
    }
}
