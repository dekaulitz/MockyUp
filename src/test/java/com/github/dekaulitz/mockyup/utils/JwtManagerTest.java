...
class JwtManagerTest {
    ...
    void validateFakeToken() {
        try {
            ...
            if (e instanceof UnathorizedAccess) {
                Assert.isTrue(((UnathorizedAccess) e).getErrorVmodel().getErrorMessage().equals(ResponseCode.TOKEN_INVALID.getErrorMessage()), "exception message is not same");
            }
        }
    }
    ...
    void validateTokenWhenTokenExpired() {
        try {
            ...
            Assert.isInstanceOf(UnathorizedAccess.class, e, "exception is not expected");
            if (e instanceof UnathorizedAccess) {
                Assert.isTrue(((UnathorizedAccess) e).getErrorVmodel().getErrorMessage().equals(ResponseCode.TOKEN_EXPIRED.getErrorMessage()), "exception message is not same");
            }
        }
    }
    ...
    void validateTokenWhenTokenExpiredAndCanRefresh() {
        try {
            ...
            Assert.isInstanceOf(UnathorizedAccess.class, e, "exception is not expected");
            if (e instanceof UnathorizedAccess) {
                Assert.isTrue(((UnathorizedAccess) e).getErrorVmodel().getErrorMessage().equals(ResponseCode.REFRESH_TOKEN_REQUIRED.getErrorMessage()), "exception message is not same");
            }
        }
    }
