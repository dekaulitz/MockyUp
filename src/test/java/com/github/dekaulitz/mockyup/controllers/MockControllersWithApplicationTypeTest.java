...
class MockControllersWithApplicationTypeTest extends BaseTest {
    ...
    void getMocksCategoriesCatIdBooksBookId() throws IOException {
        ...
        Assert.isTrue(response.getBody().equals(Helper.DEFAULT_COMPONENT_EXAMPLE), RESPONSE_BODY_NOT_EXPECTED);
        ...
        Assert.isTrue(response.getBody().contains(Helper.DEFAULT_COMPONENT_EXAMPLE), RESPONSE_BODY_NOT_EXPECTED);
    }
    ...
    void getMocksMockingDetailUpdate() throws IOException {
        MockEntities mockEntities = Helper.getMockEntities();
        when(this.mockRepository.findById(any(String.class))).thenReturn(Optional.ofNullable(mockEntities));
