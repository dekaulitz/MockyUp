...
class UserControllersTest extends BaseTest {
    ...
    void adduser() throws UnsupportedEncodingException {
        ...
        HttpEntity<RegistrationVmodel> headersHttpEntity = new HttpEntity<>(registrationVmodel, headers);
        ResponseEntity<RegistrationResponseVmodel> responseEntity = this.restTemplate
                .exchange(baseUrl + "/mocks/addUser", HttpMethod.POST, headersHttpEntity, RegistrationResponseVmodel.class);
        ...
        isTrue(response2.getBody().getResponseCode().equals(ResponseCode.USER_ALREADY_EXIST.getErrorCode()), RESPONSE_BODY_NOT_EXPECTED);
    }
    ...
    void deleteUser() throws UnsupportedEncodingException {
        ...
        ResponseEntity<Object> responseEntity = this.restTemplate
                .exchange(baseUrl + "/mocks/user/x/delete", HttpMethod.DELETE, headersHttpEntity, Object.class);
        isTrue(responseEntity.getStatusCodeValue() == 200, HTTPCODE_NOT_EXPECTED);
        ...
        ResponseEntity<ResponseVmodel> responseEntity2 = this.restTemplate
                .exchange(baseUrl + "/mocks/user/y/delete", HttpMethod.DELETE, headersHttpEntity, ResponseVmodel.class);
        isTrue(responseEntity2.getStatusCodeValue() == ResponseCode.USER_NOT_FOUND.getHttpCode().value(), HTTPCODE_NOT_EXPECTED);
        isTrue(responseEntity2.getBody().getResponseCode().equals(ResponseCode.USER_NOT_FOUND.getErrorCode()), RESPONSE_BODY_NOT_EXPECTED);
    ...
    void getUsersPagination() throws UnsupportedEncodingException {
        ...
        when(this.userRepository.paging(any(), any())).thenThrow(new RuntimeException("something bad happen"));
        headers.set("Authorization", "Bearer " + Helper.generateToken(givenId, 10, 10));
        headersHttpEntity = new HttpEntity<>(headers);
        ResponseEntity<ResponseVmodel> response = this.restTemplate
                .exchange(baseUrl + "/mocks/users", HttpMethod.GET, headersHttpEntity, ResponseVmodel.class);
        isTrue(response.getStatusCodeValue() == HttpStatus.INTERNAL_SERVER_ERROR.value(), HTTPCODE_NOT_EXPECTED);
        isTrue(response.getBody().getResponseCode().equals(ResponseCode.GLOBAL_ERROR_MESSAGE.getErrorCode()), RESPONSE_BODY_NOT_EXPECTED);
        isTrue(response.getBody().getResponseMessage().equals("something bad happen"), RESPONSE_BODY_NOT_EXPECTED);
    }

    @Test
    void getUserList() throws UnsupportedEncodingException {
        when(this.userRepository.getUserListByUserName(any(), any())).thenReturn(Helper.getUserEntitiesList());
        HttpHeaders headers = new HttpHeaders();
        headers.set("Authorization", "Bearer " + Helper.generateToken(givenId, 10, 10));
        HttpEntity<HttpHeaders> headersHttpEntity = new HttpEntity<>(headers);
        ResponseEntity<Object> responseEntity = this.restTemplate
                .exchange(baseUrl + "/mocks/users/list?username=test", HttpMethod.GET, headersHttpEntity, Object.class);
        isTrue(responseEntity.getStatusCodeValue() == 200, HTTPCODE_NOT_EXPECTED);

        //when user do request but something happen
        when(this.userRepository.getUserListByUserName(any(), any())).thenThrow(new RuntimeException("something bad"));
        headers.set("Authorization", "Bearer " + Helper.generateToken(givenId, 10, 10));
        headersHttpEntity = new HttpEntity<>(headers);
        ResponseEntity<ResponseVmodel> response = this.restTemplate
                .exchange(baseUrl + "/mocks/users/list?username=test", HttpMethod.GET, headersHttpEntity, ResponseVmodel.class);
        isTrue(response.getStatusCodeValue() == HttpStatus.INTERNAL_SERVER_ERROR.value(), HTTPCODE_NOT_EXPECTED);
        isTrue(response.getBody().getResponseMessage().equals("something bad"), RESPONSE_BODY_NOT_EXPECTED);
        isTrue(response.getBody().getResponseCode().equals(ResponseCode.GLOBAL_ERROR_MESSAGE.getErrorCode()));
    }

    @Test
    void updateUsers() throws UnsupportedEncodingException {
        when(this.userRepository.findById("y")).thenReturn(java.util.Optional.ofNullable(Helper.getUserEntities()));
        when(this.userRepository.save(any())).thenReturn(Helper.getUserEntities());
        ...
        ResponseEntity<UserEntities> responseEntity = this.restTemplate
                .exchange(baseUrl + "/mocks/users/y/update", HttpMethod.PUT, headersHttpEntity, UserEntities.class);
        isTrue(responseEntity.getStatusCodeValue() == 200, HTTPCODE_NOT_EXPECTED);
        ...
        isTrue(response.getStatusCodeValue() == ResponseCode.USER_ALREADY_EXIST.getHttpCode().value(), HTTPCODE_NOT_EXPECTED);
        isTrue(response.getBody().getResponseCode().equals(ResponseCode.USER_ALREADY_EXIST.getErrorCode()), RESPONSE_BODY_NOT_EXPECTED);

    }

    @Test
    void getUserById() throws UnsupportedEncodingException {
        when(this.userRepository.findById("x")).thenReturn(java.util.Optional.ofNullable(Helper.getUserEntities()));
        HttpHeaders headers = new HttpHeaders();
        headers.set("Authorization", "Bearer " + Helper.generateToken(givenId, 10, 10));
        HttpEntity<HttpHeaders> headersHttpEntity = new HttpEntity<>(headers);
        ResponseEntity<Object> responseEntity = this.restTemplate
                .exchange(baseUrl + "mocks/users/x/detail", HttpMethod.GET, headersHttpEntity, Object.class);
        isTrue(responseEntity.getStatusCodeValue() == 200, HTTPCODE_NOT_EXPECTED);
        ...
        ResponseEntity<ResponseVmodel> response = this.restTemplate
                .exchange(baseUrl + "mocks/users/y/detail", HttpMethod.GET, headersHttpEntity, ResponseVmodel.class);
        isTrue(response.getStatusCodeValue() == ResponseCode.USER_NOT_FOUND.getHttpCode().value(), HTTPCODE_NOT_EXPECTED);
        isTrue(response.getBody().getResponseCode().equals(ResponseCode.USER_NOT_FOUND.getErrorCode()), RESPONSE_BODY_NOT_EXPECTED);
