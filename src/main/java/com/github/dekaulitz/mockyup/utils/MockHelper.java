...
public class MockHelper {
     ...
     */
    public static MockHelper generateResponseBody(HttpServletRequest request, List<Map<String, Object>> exampleExtensions, String body, Components components) throws IOException, InvalidMockException {
        ...
        if (request.getMethod().equals(HttpMethod.POST.toString()) || request.getMethod().equals(HttpMethod.PATCH.toString())
            ...
            for (Map<String, Object> extension : exampleExtensions) {
                ...
                if (mockHelper.getResponseProperty().getMediaType().equalsIgnoreCase(XML_MEDIA_TYPE)) {
     ...
     */
    public static MockHelper generateResponsePath(HttpServletRequest request, List<Map<String, Object>> exampleExtensions, String[] openApiRoutePath, String[] paths, Components components) throws InvalidMockException {
    ...
    private static void parsingMockFromJsonMapper(MockHelper mockHelper, Map<String, Object> xExamples) {
        ...
        mockHelper.setResponseProperty(JsonMapper.mapper().convertValue(xExamples.get(RESPONSE), DtoMockResponseVmodel.class));
    }
     ...
     */
    private static void getComponentExample(MockHelper mockHelper, Components components) throws InvalidMockException {
        ...
        if (content != null && mockHelper.getResponseProperty().getMediaType() != null) {
            if (!mockHelper.getResponseProperty().getMediaType().isEmpty()) {
                if (content.get(mockHelper.getResponseProperty().getMediaType()).get(COMPONENT_REFERENCE) != null) {
                    renderingResponse(mockHelper, (String) content.get(mockHelper.getResponseProperty().getMediaType()).get(COMPONENT_REFERENCE), components);
                }else if(content.get(mockHelper.getResponseProperty().getMediaType()).get("response")!=null){
                    mockHelper.getResponseProperty().setResponse(content.get(mockHelper.getResponseProperty().getMediaType()).get("response"));
                }
            }
        } else
            ...
            if (mockHelper.getResponseProperty().get$ref() != null) {
                ...
                renderingResponse(mockHelper, (String) mockHelper.getResponseProperty().get$ref(), components);
            }
    }

    public static void renderingResponse(MockHelper mockHelper, String reff, Components components) throws InvalidMockException {
        String refName = OpenAPITools.getSimpleRef(reff);
        if (!refName.isEmpty()) {
            try {
                //injecting the component schema into mock helper response
                mockHelper.getResponseProperty().setResponse(components.getExamples().get(refName).getValue());
